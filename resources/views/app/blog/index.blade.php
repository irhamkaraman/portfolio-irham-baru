                    ...
                    <div class="card">
                        <div class="table-responsive text-nowrap">
                            <div class="card-header">
                                <a href="{{ route('blog.add') }}" class="btn btn-sm btn-primary mb-3"><i class="bx bx-plus me-1"></i> Tambah</a>
                                @session('success')
                            ...
                            <table class="table">
                                ...
                                <tbody class="table-border-bottom-0">
                                    ...
                                    <tr>
                                        <td>
                                            <img src="{{ asset('storage/blogs/' . $blog->image) }}" alt="{{ $blog->name }}" width="50">
                                            <span class="fw-medium"> {{ $blog->title }}</span>
                                        </td>

                                        <td>{{ $blog->description }}</td>
                                        <td>{{ date('l, F Y', strtotime($blog->published_at)) }}</td>

                                        <td>
                                            <a href="{{ route('blog.edit', $blog->id) }}" class="btn btn-sm btn-outline-success">Edit</a>

                                            <button type="button" class="btn btn-sm btn-outline-danger" data-bs-toggle="modal" data-bs-target="#deleteModal{{ $blog->id }}">
                                                Hapus
                                            </button>
                                            <!-- Modal -->
                                                ...
                                                <div class="modal-dialog modal-dialog-centered text-wrap text-center modal-sm" role="document">
                                                    <div class="modal-content">
                                                        <div class="modal-header">
                                                            <h5 class="modal-title" id="deleteModalLabel{{ $blog->id }}">Konfirmasi</h5>
